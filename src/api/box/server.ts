...
export async function getBox(boxId: string): Promise<any> {
  const user = await getUser();
  const notion = await getUserNotionClient(user.id);
...
export async function getAllBoxesWithStorage(): Promise<any[]> {
  const user = await getUser();
  const notion = await getUserNotionClient(user.id);
  const config = await getUserConfig();
...
export async function getBoxesByStorage(storageId: string): Promise<any[]> {
  const user = await getUser();
  const notion = await getUserNotionClient(user.id);
  const config = await getUserConfig();
